...
const main = (drawnNumbers, inputBingoCards) => {
  ...
  for (let i = 0; i < drawnNumbers.length; i++) {
    if (bCards.length === 1) {
      ...
      bCards = bCards.filter(
        (item, cardIndex) => !idToRemove.includes(cardIndex)
      )
    }
  }
  ...
  return `Winning card: ${winningCard}, Calculate score: ${winningCardScore}, Last Drawn number: ${lastNumber}, answer: ${
    winningCardScore * lastNumber
  }`
}
    ...
    bingoCard.forEach((row, rIndex) => {
      row.forEach((item, iIndex) => {
        if (item === number.toString()) {
          bingoCards[bIndex][rIndex][iIndex] =
            bingoCards[bIndex][rIndex][iIndex].concat('*')
        }
      })
    })
  })
